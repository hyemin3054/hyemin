...
Sanity CMS를 연결한 기본 개발 환경입니다. 현재 화면은 콘텐츠 조회 확인용이며 Figma 세부 디자인은 아직 구현하지 않았습니다. Node.js 22.12 이상이 필요하며 이 컴퓨터는 Node.js 24를 사용합니다.
CMS 사용법은 [Sanity 안내](docs/SANITY.md)를 참고하세요.
...
- `src/config/site.ts`: 공통 메뉴와 기본 사이트 이름
- `src/app/studio/[[...tool]]`: Sanity 관리자 화면
- `src/sanity/schemaTypes`: 콘텐츠 5종과 재사용 이미지·작품·본문 스키마
- `src/sanity/lib`: Published 조회, 타입, 이미지 URL 처리
- `sanity.config.ts`, `sanity.cli.ts`: Studio 및 CLI 설정
...
`[slug]`는 CMS에서 등록한 상세 콘텐츠의 주소 이름입니다. Published 콘텐츠만 조회하며 존재하지 않는 주소는 404로 처리합니다.
...
- 실제 Footer 연락처·운영시간은 Studio의 SITE SETTINGS에 입력하고 Publish합니다.
